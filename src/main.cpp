...
#if defined(NDEBUG)
# error "Tasnimcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x58864fbc9debc67d80f4ecf298cab17d370f1a7ea234e0f9f9abaf7c28123086");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tasnimcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tasnimcoin Signed Message:\n";
...
{
    // Tasnimcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight /  20000); // Tasnimcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tasnimcoin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // Tasnimcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tasnimcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tasnimcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe135bedc4b579541e5ab6f7fbe5d894aed538f59b9d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 01/01/2017 Birth of Tasnim";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0498416f9e2263b18efaad9ca0d781d2aa80d4f9cac57b131809b571a3ffd673a106186b82155cddbe423591a2b605265b8dc943249504ba6e6ae78131cd6c8557") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541976912;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085212203;


        if (fTestNet)
        {
            block.nTime    = 1541976747;

            block.nNonce   = 387904046;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x35d4df3af616b3ca5b636bf483bd075ba1c6ffce08d3c815bdd48550da5e02f9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Tasnimcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TasnimcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TasnimcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TasnimcoinMiner(CWallet *pwallet)
{
    printf("TasnimcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tasnimcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TasnimcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TasnimcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TasnimcoinMiner, pwallet));
}
